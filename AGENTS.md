...
- For a new standard CRUD module, follow the CLI workflow in `docs/feature_modules.md`. If the table is not yet present in the configured development database, ask for approval to apply the task's schema there; after approval, apply it and run the built-in scaffolder. Do not fall back to manual generation solely because the table is initially absent.
