...
}', '"@demo-spages-checklist"', CAST((SELECT id FROM att WHERE icode='demo-spages-checklist') AS CHAR)) AS content_json) demo
WHERE NOT EXISTS (
  SELECT 1 FROM spages
  WHERE LOWER(url)='demo-start-here'
    OR LOWER(JSON_UNQUOTE(JSON_EXTRACT(CASE WHEN JSON_VALID(draft_json) THEN draft_json ELSE NULL END, '$.url')))='demo-start-here'
);
...
WHERE NOT EXISTS (
  ...
  WHERE LOWER(url)='demo-bulletin'
    OR LOWER(JSON_UNQUOTE(JSON_EXTRACT(CASE WHEN JSON_VALID(draft_json) THEN draft_json ELSE NULL END, '$.url')))='demo-bulletin'
);
...
}', '"@demo-spages-workshop"', CAST((SELECT id FROM att WHERE icode='demo-spages-workshop') AS CHAR)) AS content_json) demo
WHERE NOT EXISTS (
  SELECT 1 FROM spages
  WHERE LOWER(url)='demo-field-notes'
    OR LOWER(JSON_UNQUOTE(JSON_EXTRACT(CASE WHEN JSON_VALID(draft_json) THEN draft_json ELSE NULL END, '$.url')))='demo-field-notes'
);
