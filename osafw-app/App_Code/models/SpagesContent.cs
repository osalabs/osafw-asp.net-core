...
{
    public const int MAX_CHARACTERS = 1_000_000;
    public const int MAX_BLOCKS = 300;
    ...
    public sealed record RenderContext(Func<string, string>? Snippet = null, Func<int, string>? Attachment = null, bool IsPublishing = false, Func<int, string>? ImageAttachment = null);
    ...
    public static JsonObject parse(string json)
    {
        ...
        {
            throw new UserException("Page content is too large.");
        }

        JsonObject doc;
        try
        {
            doc = JsonNode.Parse(json, documentOptions: new JsonDocumentOptions { MaxDepth = 32 }) as JsonObject ?? throw new JsonException();
        }
        catch (JsonException)
        {
            throw new UserException("Invalid page content. Your unsaved changes have been kept in the editor.");
        }
        ...
        foreach (var region in regions)
        {
            ...
            foreach (var block in blocks)
            {
                ...
                else if (text(b, "type")is "paragraph" or "header" or "quote" or "callout")
                {
                    ...
                    if (data["text"] != null)
                    {
                        data["text"] = inline(text(data, "text"));
                    }
                }
            }
        }
        ...
        if (count > MAX_BLOCKS)
        {
            throw new UserException("A page can contain at most 300 blocks.");
        }

        if (doc["slots"] != null && doc["slots"] is not JsonObject)
        {
            throw new UserException("Invalid snippet slots.");
        }
    ...
    public static string renderRegion(JsonObject document, string region, RenderContext context)
    {
        ...
        if (document["regions"]?[region]?["blocks"] is not JsonArray blocks)
        {
            return "";
        }
        ...
        foreach (var block in blocks.OfType<JsonObject>())
        {
            ...
            if (!Tools.TryGetValue(type, out var render))
            {
                throw new UserException($"The '{type}' block needs an installed server renderer before it can be previewed or published.");
            }
    ...
    public static void validate(JsonObject document, string layoutName, RenderContext context, bool isSnippet)
    {
        ...
        if (isSnippet && layoutName != "article")
        {
        ...
        if (isSnippet && listSnippetKeys(document).Count > 0)
        {
        ...
        if (document["slots"] is JsonObject slots)
        {
            ...
            {
                if (!definition.Slots.Contains(slot.Key))
                {
                    throw new UserException("This layout does not declare the selected snippet slot.");
                }

                if (!string.IsNullOrEmpty(slot.Value?.ToString()))
                {
                    context.Snippet?.Invoke(slot.Value!.ToString());
                }
            }
        }
    }

    public static HashSet<string> listSnippetKeys(JsonObject document)
    {
    ...
    public static HashSet<int> listAttachmentIds(JsonObject document)
    {
    ...
    public static JsonObject fromMarkdown(FwDict page, Func<string, int>? resolveAttachment = null)
    {
        ...
        )
        {
            ...
            bool isKeepRegion = Regex.IsMatch(source, @"(?m)^\s{0,3}\[[^\]]+\]:") || parsed.Any(x => x.Span.Start < 0 || x.Span.End >= source.Length);
            if (isKeepRegion)
            {
                blocks.Add(new JsonObject { ["type"] = "legacyMarkdown", ["data"] = new JsonObject { ["text"] = source } });
            }

            foreach (var node in isKeepRegion ? Enumerable.Empty<Block>() : parsed)
            {
                ...
                if (node is HeadingBlock heading && heading.Level >= 2)
                {
                    ...
                    data = new()
                    {
                        ["text"] = inline(markdown(Regex.Replace(original, @"^\s{0,3}#{1,6}\s+|\s+#+\s*$", ""))),
                        ["level"] = heading.Level
                    };
                }
                ...
                if (node is ParagraphBlock media && media.Inline?.FirstChild is LinkInline link && link.NextSibling == null && resolveAttachment?.Invoke(link.Url ?? "")is int attachmentId && attachmentId > 0)
                {
                    ...
                    data = link.IsImage ? new()
                    {
                        ["att_id"] = attachmentId,
                        ["alt"] = html.QuerySelector("img")?.GetAttribute("alt") ?? "",
                        ["decorative"] = false,
                        ["caption"] = ""
                    }
                    ...
                    {
                        ["att_id"] = attachmentId,
                        ["title"] = html.QuerySelector("a")?.TextContent ?? "Download file"
                    };
                }
                ...
                else if (node is QuoteBlock quote && quote.Count == 1 && quote[0] is ParagraphBlock)
                {
                    ...
                    data = new()
                    {
                        ["text"] = html.QuerySelector("blockquote p")?.InnerHtml ?? original,
                        ["caption"] = ""
                    };
                }
                else if (node is Markdig.Extensions.Tables.Table)
                {
                    ...
                    foreach (var row in html.QuerySelectorAll("tr"))
                    {
                        rows.Add(new JsonArray(row.Children.Select(cell => JsonValue.Create(cell.InnerHtml)).ToArray()));
                    }
            ...
            regions[pair.Item1] = new JsonObject
            {
                ["blocks"] = blocks
            };
        }
    ...
    public static string safeUrl(string value)
    {
        ...
        if (value.Length == 0 || value.Any(char.IsControl) || value.Contains('\\') || value.StartsWith("//"))
        {
            throw new UserException("Enter a safe link URL.");
        }
        ...
        if (Uri.TryCreate(value, UriKind.Absolute, out var uri) && uri.Scheme is "http" or "https" or "mailto" or "tel")
        {
            return value;
        }
    ...
    private static HtmlSanitizer makeSanitizer(bool isInlineOnly)
    {
        ...
        s.AllowedTags.Clear();
        foreach (var tag in (isInlineOnly ? "b strong i em a code mark br s u sub sup" : "p h1 h2 h3 h4 h5 h6 b strong i em a code pre mark br s u sub sup ul ol li blockquote hr table thead tbody tr th td img figure figcaption input").Split(' '))
        {
            s.AllowedTags.Add(tag);
        }

        s.AllowedAttributes.Clear();
        foreach (var attr in "href title alt src colspan rowspan scope type checked disabled".Split(' '))
        {
            s.AllowedAttributes.Add(attr);
        }

        s.AllowedSchemes.Clear();
        foreach (var scheme in "http https mailto tel".Split(' '))
        {
    ...
    private static string renderList(JsonObject data, RenderContext context)
    {
        ...
        {
            throw new UserException("A list needs items.");
        }

        if (items.Any(x => x is not JsonValue))
        {
            throw new UserException("Version-one lists contain text items. Use Markdown for nested lists.");
        }
    ...
    private static string renderHeader(JsonObject data, RenderContext context)
    {
        ...
        if (level < 2 || level > 6)
        {
            throw new UserException("Content headings must be H2–H6; the page title is H1.");
        }

        string anchor = text(data, "anchor");
        if (anchor.Length > 0 && !Regex.IsMatch(anchor, @"^[a-zA-Z][a-zA-Z0-9_-]{0,63}$"))
        {
            throw new UserException("Heading anchors start with a letter and contain letters, numbers, hyphens, or underscores.");
        }
    ...
    {
        bool isDecorative = text(data, "decorative") == "true";
        string alt = text(data, "alt");
        if (context.IsPublishing && !isDecorative && string.IsNullOrWhiteSpace(alt))
        {
            throw new UserException("Add image alternative text or mark the image as decorative.");
        }
        ...
        if (url.Length == 0)
        {
            ...
            {
                throw new UserException("Select an available image attachment.");
            }

            return "";
        }

        return "<figure><img loading=\"lazy\" class=\"img-fluid\" src=\"" + escape(url) + "\" alt=\"" + escape(isDecorative ? "" : alt) + "\"><figcaption>" + inline(text(data, "caption")) + "</figcaption></figure>";
    }
    ...
    private static string renderFile(JsonObject data, RenderContext context)
    {
        ...
        if (url.Length == 0)
        {
            if (context.IsPublishing)
            {
                throw new UserException("Select an available file attachment.");
            }
    ...
    {
        if (data["content"] is not JsonArray rows)
        {
            throw new UserException("A table needs rows.");
        }

        if (context.IsPublishing && (text(data, "withHeadings") != "true" || string.IsNullOrWhiteSpace(text(data, "caption"))))
        {
            throw new UserException("Add a table caption and column headings.");
        }
        ...
        {
            if (rows[i] is not JsonArray cells)
            {
                throw new UserException("Invalid table row.");
            }

            bool isHeader = i == 0 && text(data, "withHeadings") == "true";
            html.Append("<tr>");
            foreach (var cell in cells)
            {
                html.Append(isHeader ? "<th scope=\"col\">" : "<td>").Append(inline(cell?.ToString() ?? "")).Append(isHeader ? "</th>" : "</td>");
            }
    ...
    {
        if (data["items"] is not JsonArray items || items.Count > 12)
        {
            throw new UserException("Use at most 12 cards in a group.");
        }
        ...
        foreach (var item in items.OfType<JsonObject>())
        {
            ...
            if (text(item, "url").Length > 0)
            {
                html.Append("<a href=\"").Append(escape(safeUrl(text(item, "url")))).Append("\">").Append(escape(required(item, "label", "Card link label"))).Append("</a>");
            }
