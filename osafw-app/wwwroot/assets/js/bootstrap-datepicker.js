 ...
 *
 * !!!!!!!!!!!!
 * 2013-04-13 osa - differnet component handling - button added for components, not just .add-on
 	- fixed empty value - and data-date not used in component now
...
!function( $ ) {
	...
	var Datepicker = function(element, options){
		...
		this.autoclose = options.autoclose ? true : false;
		...
		var button = this.element.next('button');
		this.component = this.element.is('.date') ? this.element.find('button, .add-on') : false;
		...
		if (this.isInput) {
			...
			if (this.component){
				...
				this.element.find('input').on('click', $.proxy(this.toggle, this));
			} else {
	...
	Datepicker.prototype = {
		...
		show: function(e) {
			...
			$(document).on('mousedown', $.proxy(this.hide_mousedown, this));
		...
		hide: function(){
			...
			this.showMode();
			if (!this.isInput) {
				$(document).off('mousedown', this.hide_mousedown);
			}
			//this.set();
			this.element.trigger({
				type: 'hide',
		...
		update: function(newDate){
			this.date = DPGlobal.parseDate(
				...
				typeof newDate === 'string' ? newDate : (this.isInput ? this.element.prop('value') : this.element.find('input').prop('value')),
				this.format
			);
			this.viewDate = new Date(this.date.getFullYear(), this.date.getMonth(), 1, 0, 0, 0, 0);
			this.fill();
				...
				switch(target[0].nodeName.toLowerCase()) {
					...
					case 'td':
						if (target.is('.day') && !target.is('.disabled')){
							...
							if (this.autoclose && this.viewMode === 0 ) {
								this.hide();
							}
						}
						break;
