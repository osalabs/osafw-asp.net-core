...
- Additive developer-tooling change: full database schema metadata now exposes neutral column comments and correctly isolates schema-qualified SQL Server tables; cold OLE metadata reads open their connection without changing warm-cache behavior. Newly scaffolded typed `Row` classes map signed `bigint` to `long`, unsigned `bigint` to `ulong`, and decimal/numeric columns to `decimal`, emit escaped XML docs from column comments, and generate safe `[DBName]` mappings for normalized identifiers. Existing generated source is unchanged until it is deliberately regenerated.
