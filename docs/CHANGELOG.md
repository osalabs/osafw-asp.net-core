...
- Breaking: a non-empty `settings.test_email` now takes precedence over `appSettings.test_email` in test mode. Verify the effective recipient after upgrading; blank retains the configuration fallback, and `current_user` routes to the signed-in user. `FW.sendEmail()` now returns false for missing recipients or unusable SMTP host/port settings; callers must handle failure. See [test email settings](settings.md).
...
- Breaking: the sample `Development` override now sets `appSettings.log_pii=true`. Apps copying this override should set it to false if local debug logs must continue redacting SQL parameters and other guarded context.
...
- Breaking: the default attachment Open path now serves trusted PDFs inline. Apps requiring forced PDF downloads should link to `/Att/Download/{icode}`.
