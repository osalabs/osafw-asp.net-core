...
- Vue interaction update: apply the provider's additive `upd2026-09-08-user-view-widths.sql` before using saved widths in an existing database. Lists gain accessible width controls and remembered filter visibility; forms gain neutral validation issues and permission-aware controls. `immutable_on_edit` ignores existing-row changes in standard save paths. Quick-edit context retention is opt-in and keeps existing save triggers. See [Vue interaction behavior](dynamic.md#vue-interaction-behavior).
- Additive list contract: Dynamic and Vue controllers now support `list_calculated_fields` metadata for virtual list columns. Declare each calculated field's source dependencies so selected list/search/export queries stay lean; calculated fields are not searchable or automatically sortable, calculation-only source fields are omitted from Vue JSON, and explicit database-backed `list_sortmap` entries remain supported. See [Calculated list columns](dynamic.md#calculated-list-columns).
- Additive developer-tooling change: full database schema metadata now exposes neutral column comments and correctly isolates schema-qualified SQL Server tables; cold OLE metadata reads open their connection without changing warm-cache behavior. Newly scaffolded typed `Row` classes map signed `bigint` to `long`, unsigned `bigint` to `ulong`, and decimal/numeric columns to `decimal`, declare nullable text as `string?`, emit escaped XML docs from column comments, and generate safe `[DBName]` mappings for normalized identifiers. Existing generated source is unchanged until it is deliberately regenerated.
- Additive developer-tooling change: opt-in `isRowRegeneration` builds expose a Site Admin, development-only POST at `/Dev/Manage/(RegenerateModelRows)` to refresh all existing model `Row` classes. Entire Rows, including custom members, are replaced; review or revert the generated files in Git. The action and its conditional `Microsoft.CodeAnalysis.CSharp` dependency are absent from normal builds. Copied applications adopting the tool must also copy the conditional package reference. See [Row regeneration](feature_modules.md#refresh-existing-typed-rows).
