...
- Additive developer-tooling change: Developer Tools can preview and explicitly apply current schema metadata to selected model `Row` classes. The source-aware replacement preserves the model's inheritance and all source outside a recognizable generated-style `Row`, rejects customized or ambiguous Rows, and rejects a stale preview before writing. Copied applications adopting this tool must also add `Microsoft.CodeAnalysis.CSharp` 5.9.0 to their application project.
