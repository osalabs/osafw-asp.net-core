...
The same option is available on `ConvUtils.html2pdf` and `parsePagePdf`. Supply it only from trusted server code, never a submitted path. Use a dedicated directory containing only public static assets; do not include uploads, secrets or configuration files, and do not allow untrusted processes to replace its files or directories during rendering.
