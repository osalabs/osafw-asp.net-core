...
In local asset mode, PDFs are staged beside the destination and replace it only after successful rendering; failed renders preserve existing files and attempt to delete their own partial output. Browser-download temporary PDFs are deleted after `fileResponse` finishes when possible. Deletion errors are ignored so cleanup cannot fail a download or replace the original render error. Downloads also call `Utils.cleanupTmpFiles()` to retry framework temporary files older than one hour, including leftovers from interrupted requests; this sweep does not scan arbitrary report destination directories. No temporary HTML file is needed. Existing callers without the option retain normal HTML rendering and write directly to the destination, including existing files.
