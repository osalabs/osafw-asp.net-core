...
A bounded, one-time repair and verification harness are prepared under ignored `artifacts/assistant_spages_schema_repair/`. They are deliberately outside normal application updates: the preview was unpublished, and the developer requested no permanent compatibility layer. The repair maps the revised columns and snapshot metadata, preserves content and revision identities, converts revision creation timestamps from the preview's UTC default to the database timezone, and removes the empty retired redirects table. It accepts only draft pages and saved revisions, rejects populated redirects or retired image metadata, locks affected tables, and rolls back on failure. The original update ledger is preserved. The two unconverted original pages remain available for the existing explicit conversion action.
