...
- Tightened `FwReports` initialization and rendering by defaulting filters/format to empty strings, guarding sort defaults and export filenames, and tolerating empty data when exporting.
...
- Guarded configuration buckets and validation helpers so `FwConfig.settings` and `validateCheckResult` no longer unbox null values under nullable analysis.
- Initialized test DTO strings and controller ShowForm results to safe defaults and relaxed key/value reflection caches to allow nullable returns, trimming another batch of analyzer warnings.
...
- Latest build with `/root/dotnet/dotnet build /p:LibraryRestore=false /clp:Summary` now reports 185 nullable warnings after today’s fixes cleared constructor/default warnings in FW/FwLogger/ParsePage.
