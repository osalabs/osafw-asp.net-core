...
- Final normal: `dotnet test osafw-tests/osafw-tests.csproj -p:NuGetAuditMode=all --filter 'FullyQualifiedName!~osafw.Tests.DBTests' --logger 'trx;LogFileName=normal-updated.trx' --results-directory artifacts/assistant_testability/revision-test-results/normal` — 713 passed.
- Final SQLite: `dotnet test osafw-tests/osafw-tests.csproj '-p:DefineConstants=TRACE%3BDEBUG%3BisSQLite' -p:NuGetAuditMode=all --filter 'FullyQualifiedName!~osafw.Tests.DBTests' --logger 'trx;LogFileName=sqlite-updated.trx' --results-directory artifacts/assistant_testability/revision-test-results/sqlite` — 724 passed.
