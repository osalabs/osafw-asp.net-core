...
After adding the prerequisite's classic Dynamic action/export regressions, the final integration command was `dotnet test osafw-tests/osafw-tests.csproj --no-restore --filter 'FullyQualifiedName~CalculatedDynamicListTests|FullyQualifiedName~VueInteractionBrowserTests|FullyQualifiedName~VueInteractionBackendTests|FullyQualifiedName~FwVueControllerTests|FullyQualifiedName~FwVirtualControllerTests|FullyQualifiedName~FwDynamicControllerColumnFilterTests' --logger 'console;verbosity=minimal'`: 77 passed, none skipped, including all 24 browser cases. Runtime Vue behavior is unchanged by that test-only prerequisite follow-up.

## Review feedback follow-up (2026-09-21)

Merged current master into `codex/vue-interaction-contracts`; the only conflict was the task index, resolved by retaining both additive entries. GitHub rejected reopening closed PR #294 and changing its base. A replacement PR targets master from the same feature branch; no merge is authorized for this follow-up.

Renamed the unreleased `immutable_on_edit` field option to `is_edit_readonly`, including standard main/child save handling, Vue controls, examples, tests and canonical documentation. The field remains writable on creation; standard edits ignore submitted replacement values. Direct model writes and custom actions remain outside this option's enforcement. No compatibility alias was added for the unreleased draft name.

The naming comparison used FPF F.18 at pinned revision `083cdb15580906da7b94c36f613bf57358550b53`: [Local-First Unification Naming Protocol](https://github.com/ailev/FPF/blob/083cdb15580906da7b94c36f613bf57358550b53/FPF-Spec.md#L107542), Anatoly Levenchuk, CC BY 4.0. Plain-language decision: `is_edit_readonly` names the actual edit-only scope; `is_immutable` suggests domain-wide permanence, `is_readonly` loses the create/edit distinction, and `is_ro` is less clear. Revisit the name if the enforcement scope changes. This is an original application of the naming guidance, not copied specification text.

Built-in validation messages now come from `common/vue/validation-messages.sel` and the current template language. The initial HTML supplies the same translated dictionary for legacy frontend error codes; structured responses keep their existing message shape and preserve custom application messages. Summary labels use template language markers, and the shipped Chinese dictionary includes the nine validation/summary phrases. JSON encoding and Vue text interpolation preserve quotes and HTML-like message text without executing it. Copied applications must take the new template alongside the controller/template updates, as documented in the changelog.

Read-only tracing confirmed that without `isRoles`, `Users.getRBAC()` returns all default permissions and capabilities still respect read-only restrictions. The broader question of moving structured issues into Dynamic/base controllers remains discussion only; no validation architecture or RBAC behavior was changed.

Verification: the combined integration command recorded above passed 79 tests, none failed/skipped, including 25 offline browser cases. Added checks cover Chinese built-in messages in structured errors and the actual HTML action state, custom-message preservation, legacy frontend fallbacks and safe text rendering. Existing creation/edit, child-row and save-coordination regressions passed. `git diff --check` passed. No live application, database/provider integration, or deployment was exercised. Fresh independent review and supplemental summary/index audit completed with no blocking findings after correcting the shipped Chinese translation gap. The final rerun retained all 79 passing tests; UTF-8/no-BOM/CRLF checks passed for all 16 changed/new files. Review loop can stop.
