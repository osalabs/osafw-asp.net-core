...
- Final Chrome capture found nested Delete icons could toggle row selection through the row click handler. Checking the nearest interactive ancestor preserves selection on failed deletes while ordinary cell clicks still toggle it.
- Bumped `SITE_VERSION` to `0.26.0922.2` for the stylesheet change. No breaking upgrade entry is needed.

## Verification

All **52 Vue browser cases passed, zero failures/skips**, with real Bootstrap and site CSS. Two added cases cover left/right controls columns, link bounds, configured actions, forwarded slots, nested Delete-icon clicks and retained selection after failure, read-only pointer and Enter-key disabling, and unchanged no-width layout. The fixture retains nested icon hit targeting using a span with a multiplication character. An initial exact-label selector was corrected to allow the fixture's untranslated backtick delimiters. Isolated output compiled the app and tests without replacing Visual Studio's app binaries.

```powershell
$taskRoot = Join-Path (Get-Location).Path 'artifacts/assistant_vue297'
$env:PLAYWRIGHT_BROWSERS_PATH = "$taskRoot/browsers"
dotnet test osafw-tests/osafw-tests.csproj --no-restore -p:OutDir="$taskRoot/build/default/" --filter 'FullyQualifiedName~VueInteractionBrowserTests' --logger 'console;verbosity=minimal' --logger 'trx;LogFileName=acceptance-icon-reviewed.trx' --results-directory artifacts/assistant_vue297/test-results/acceptance
git diff --check
```
...
The gallery contains 25 Chrome screenshots, an acceptance checklist, and four interactive demonstrations in ignored task artifacts. Controlled demos use current framework components with simulated responses; they do not establish backend authorization or database behavior. Chrome confirmed queued tab saves, retained errors and recovery, read-only fields and capabilities, calculated columns, wrapped action links, failed-delete selection retention, and quick-edit focus/selection/scroll preservation after both successful and failed list refreshes. Ordinary row-cell clicks still toggle selection. Visual Studio MCP was unreachable; the existing app was used without starting another IIS process. The running app retained its previously loaded asset version, so current-source browser fixtures provide the final CSS verification.
