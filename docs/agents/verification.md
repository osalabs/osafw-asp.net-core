...
| --- | --- | --- |
| Agent/docs-only | Validate links/routes, strict UTF-8/CRLF, summary index entry, and representative prompt routing. Check commands/examples against real paths. | Integrating review plus the `agent-workflow` overlay for shared workflow changes; use an independent reviewer when useful/available or the documented local fallback. |
