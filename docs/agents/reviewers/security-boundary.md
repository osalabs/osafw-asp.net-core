...
- User/stored/editor HTML and markdown are escaped or sanitized before output. `noescape`, raw markdown HTML, Vue `v-html`, and HTML email content require server-controlled or already-sanitized data. For default-deny changes, inventory both attacker-controlled sinks and intended server-trusted consumers, then test negative execution cases and positive trusted compatibility through the established entry paths.
