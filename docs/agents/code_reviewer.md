...
6. **Project fit/simplicity:** Does the change follow nearby osafw controller/model/template/config patterns and canonical docs with the fewest justified moving parts? Check constant names, boolean prefixes, and readable logical blocks against `docs/naming.md`. Flag wrappers, test-only seams, duplicate branches, new defaults/casts, or restating comments only when they create real cost/risk.
