...
The editor keeps Content, Navigation and Search, Image, Custom, and Revisions in top tabs; executable customization remains restricted to Site Admins. Save draft and Preview are in the header. Navigation and Search groups URL/access, navigation, search engines, and redirects. The URL prefix follows the selected parent’s working-draft path. The Image tab uses the standard Select / Upload modal, with a recommended 1110 × 300 px page image. Its library is limited to this page’s images and unbound public images; uploads are attached to the current page. Removing the selected page image clears the draft reference and does not delete the uploaded file.
